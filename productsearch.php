...
$user_email="";
session_start();
$v=$_SESSION['searchValue'];
    $conn = mysqli_connect('localhost','root','','myshop');
if($conn){
    $insert = "SELECT * FROM products WHERE name LIKE '%$v%' OR about LIKE '%$v'";
...
if($conn){
    ...
    $read2 = "SELECT * FROM cart WHERE productid = '$id' AND usermail='$useremail'";
...
<?php include './components/header.php'; ?>
    ...
    <section class="container">
      ...
      <?php
if(empty($result) || $result->num_rows <= 0){
    echo "<h2 class='text-center bg-warning  fw-bold mt-4'>No result found</h2>";
 }
                ...
                <div class='product-details'>
                    ...
                    <div class='ratting-info'>
                        ...
                        <input type='text' name='id' value='{$row['id']}' hidden/>
                            <button name='addtocart' style='width:100%' name='addtocart'  class='bg-primary btn'> <a   style='text-decoration: none; color: aliceblue; padding: 5px; border-radius: 10px; '> Add to cart</a> </button></form>   </div>
                    </div>
